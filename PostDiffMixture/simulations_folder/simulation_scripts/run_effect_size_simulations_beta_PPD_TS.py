...
#        print("output_file cols", output_file.columns)
...
#                 print("Forcing actions:", forceActions)
    ...
    #        thresh = 0.1 # for small effect, es = 0.1, 0.55 - 0.45 = 0.10
            ppd.calculate_epsilon_single_bandit(reordered_reward_file, models = models, 
                                         num_actions=len(prob_per_arm), 
                                         dest= cur_output_file,
                                         forced = forced, c=c, resample = resample)
...
#         print("Calculated nobs for effect size:", nobs_total)
    ...
    if len(sys.argv) > 7 and sys.argv[7].startswith("forceActions"):
        ...
        num_to_force = 1 #force 1 action from each arm to avoid nan means
        ...
        if bandit_type == "uniform":
            ...
            if len(sys.argv) > 5:
                ...
                run_simulations(num_sims, prob_per_arm, step_sizes, outfile_directory, 
                                ...
                                forceActions = num_to_force, batch_size = batch_size, burn_in_size = burn_in_size, c=c, resample = resample)
            else:
                run_simulations(num_sims, prob_per_arm, step_sizes, outfile_directory, forceActions = num_to_force, batch_size = batch_size, \
                    burn_in_size = burn_in_size, c = c, resample = resample)
