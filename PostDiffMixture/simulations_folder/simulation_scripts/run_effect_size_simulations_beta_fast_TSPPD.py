...
def make_stats_row_from_df(simulations_df, include_power, action_count, effect_size = None, alpha = None, step_size = "JEDM"):
    ...
    elif step_size == "FullHistory":
        step_size = max(simulations_df.index) + 1 #will be 4n for instance
        n_size_list = np.arange(2, int(step_size + 1))#2 to 4*n  
...
def calculate_statistics_from_sims(simulations_dfs_list, effect_size,
    action_count, alpha = 0.05, step_size = "JEDM"):
    '''
    ...
    for simulations_df in simulations_dfs_list:
        stats_rows_list = make_stats_row_from_df(simulations_df, True,
            action_count, effect_size, alpha, step_size = step_size)
...
def main():
    ...
    ns_stop = 0
    if "NSStop" in outfile_directory:
#        ns_stop = 100
      #  ns_stop = 128
        ns_stop = 404
    print("Resampling?", resample)
    print("NSStop", ns_stop)
    #batch_size = 1.0
 #   if len(sys.argv) > 5:
           ...
           # results_dfs_list, results_output_names = run_simulations(
            ...
            if len(sys.argv) > 5:
                ...
                results_dfs_list, results_output_names = run_simulations(
                    ...
                    random_start=random_start_r, c=c, resample = resample, ns_stop = ns_stop)
            else:
                run_simulations(num_sims, prob_per_arm, step_sizes,
                    outfile_directory, forceActions = num_to_force,
                    batch_size = batch_size, burn_in_size = burn_in_size, c=c, resample = resample, ns_stop = ns_stop)
...
#        results_df.to_csv('{}_sims={}_m={}.csv.gz'.format(results_output_name, num_sims, random_dur_m), compression = "gzip", index=False)
    ...
    if "FullHistory" in outfile_directory:
        step_size_st = "FullHistory"
    else:
        step_size_st = "JEDM"

    stats_df = calculate_statistics_from_sims(results_dfs_list,effect_size, num_arms, alpha = 0.05, step_size = step_size_st)
