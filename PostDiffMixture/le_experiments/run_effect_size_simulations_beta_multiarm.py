...
def calculate_by_trial_statistics_from_sims(outfile_directory, num_sims, step_sizes, alpha = 0.05, effect_size = None):
    '''
...
def make_stats_row_from_df(cur_data, include_power, alpha = None, num_arms = 4, effect_size = None):
    ...
    '''
    sample_sizes = np.array([np.sum(cur_data[action_header] == i) for i in range(1,1 + num_arms)])
    ...
    mean_control = np.mean(cur_data[cur_data[action_header] == 1][obs_reward_header])# JN mean for arm 1 which is control
    sample_size_control = sample_sizes[0]
    cur_row['sample_size_1'] = sample_size_control
    cur_row['mean_1'] = mean_control

    wald_stat_list = []
    for i in range(1, num_arms):
        # Iterate over all other arms besides control, so 1 to 3; range(1, ), sample_sizes is python list so counts from 0, 1,2,3 are idxs of interets

        sample_size_i = sample_sizes[i]
        #sample_size_2 = sample_sizes[1]
        cur_row['sample_size_{}'.format(i + 1)] = sample_size_i #calling arm 1 control as per curr data
        #cur_row['sample_size_2'] = sample_size_2

        mean_i = np.mean(cur_data[cur_data[action_header] == i + 1][obs_reward_header])# JN mean for arm 1
        #mean_2 = np.mean(cur_data[cur_data[action_header] == 2][obs_reward_header])#JN mean for arm 2

        cur_row['mean_{}'.format(i + 1)] = mean_i
        #cur_row['mean_2'] = mean_2
        #SE = sqrt[(P^hat_A*(1-P^hat_A)/N_A + (P^hat_B*(1-P^hat_B)/N_B]
        SE = np.sqrt(mean_i*(1 - mean_i)/sample_size_i + mean_control*(1 - mean_control)/sample_size_control)
        wald_type_stat_i = (mean_i - mean_control)/SE #(P^hat_A - P^hat_b)/SE
        ...
        cur_row['wald_type_stat_{}'.format(i + 1)] = wald_type_stat_i

        wald_stat_list.append(wald_type_stat_i)

        #cur_row['wald_pval'] = wald_pval
        #print("wald_pval", wald_pval)

    wald_stat_list = np.array(wald_stat_list)
    max_z = np.max(wald_stat_list)
    cur_row['max_wald_stat'] = max_z
    reject_null = max_z > C_ALPHA
    cur_row['reject_null_wald'] = reject_null
...
def calculate_statistics_from_sims(outfile_directory, num_sims, step_sizes, alpha = 0.05, num_arms = 4, effect_size = None):
    '''
    ...
    for num_steps in step_sizes: 
        for i in range(num_sims):
            output_file = pd.read_csv( get_output_filename(outfile_directory, num_steps, i),skiprows=1)
            cur_row = make_stats_row_from_df(output_file, True, alpha, num_arms)
            cur_row['num_steps'] = num_steps
            cur_row['sim'] = i
                                                                   
            rows.append(cur_row)
    # Now do some aggregating
    sample_size_list = ["sample_size_{}".format(i) for i in range(1,num_arms + 1)]
    mean_list = ["mean_{}".format(i) for i in range(1,num_arms + 1)]
    wald_type_stat_list = ["wald_type_stat_{}".format(i) for i in range(2,num_arms + 1)]

    df = pd.DataFrame(rows, columns=['num_steps', 'sim',\
     'total_reward', 'reject_null_wald', 'max_wald_stat'] + sample_size_list + mean_list + wald_type_stat_list)
...
#                 print("Forcing actions:", forceActions)
            ...
            models = [beta_bernoulli.BetaBern(success=successPrior, failure=failurePrior) for _ in range(len(prob_per_arm))] #model per arm, so just need prob per arm
...
#         print("Calculated nobs for effect size:", nobs_total)
    ...
    outfile_prefix = outfile_directory  + bandit_type_prefix
    # if effect_size == 0:
    #     # Then include the n  in the prefix
    #     outfile_prefix += "N" + str(n) 

    df = calculate_statistics_from_sims(outfile_directory, num_sims, step_sizes, DESIRED_ALPHA, num_arms)
    df.to_pickle(outfile_prefix + 'Df.pkl')
    df_by_trial = calculate_by_trial_statistics_from_sims(outfile_directory, num_sims, step_sizes, DESIRED_ALPHA)
